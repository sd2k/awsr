...
aws_key = function() Sys.getenv('AWS_ACCESS_KEY_ID')
aws_secret = function() Sys.getenv('AWS_SECRET_ACCESS_KEY')

# Miscellaneous functions to format time and date
now_s3 <- function() format(lubridate::now(), '%Y%m%dT%H%M%SZ')
today_s3 <- function() format(lubridate::today(), '%Y%m%d')
...
create_request = function(request_method, headers, payload,
  ...
  hashed_payload <- digest(payload, algo="sha256", serialize = FALSE)
  ...
  canonical_request <- str_c(request_method, '\n',
                            ...
                            signed_headers, '\n',
                            hashed_payload)
  hashed_canonical_request <- digest(canonical_request, algo="sha256", serialize = FALSE)
...
#' @importFrom stringr str_c
#' @examples
#' create_credential_scope('20110909', 'us-east-1', 'iam')
create_credential_scope = function(region, service, date_stamp) {
...
#'  of the same name
#' @param request_time string containing the date and time of the request,
...
#'                '20110909T233600Z\n')
create_string_to_sign = function(full_canonical_request, credential_scope,
                                 request_time) {
...
create_signing_key = function(region_name, service_name,
                              date_stamp = today_s3()) {

  key_date = hmac(str_c('AWS4', aws_secret()), date_stamp, algo = 'sha256',
                  raw = TRUE)
  key_region = hmac(key_date, region_name, algo = 'sha256', raw = TRUE)
  key_service = hmac(key_region, service_name, algo = 'sha256', raw = TRUE)
  key_signing = hmac(key_service, 'aws4_request', algo = 'sha256', raw = TRUE)
  key_signing
}
...
#'  \code{host} header. See examples for correct format
#' @param payload the payload from the body of the HTTP/HTTPS request
#' @param endpoint the AWS endpoint to send the request to
#' @param query the query parameters of the request, url encoded
#' @param date_stamp request date in the form YYYYMMDD; defaults to the current
#'  date.
#' @param region_name name of the AWS region being targeted, e.g. 'eu-west-1'
#' @param service_name name of the AWS service being targeted, e.g. 'ec2'
...
#' @importFrom digest hmac
#' @importFrom stringr str_c
#' @return authorisation string to be included in request headers
create_auth <- function(request_method, headers, payload, uri, query,
  ...
  credential_scope <- create_credential_scope(region_name, service_name, date_stamp)
  string_to_sign <- create_string_to_sign(full_request, credential_scope, request_time)

  signing_key <- create_signing_key(region_name, service_name, date_stamp)

  signature <- hmac(signing_key, string_to_sign, algo="sha256")
